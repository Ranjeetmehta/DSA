...
class Solution {
	int rec(String ceo,HashMap<String,ArrayList<String>> mngrAndDidirect, TreeMap<String,Integer> ans){
		if(mngrAndDidirect.containsKey(ceo)==false){
			ans.put(ceo,0);
			return 1;
		}
		int cnt=0;
		for(String emp:mngrAndDidirect.get(ceo)){
			cnt+=rec(emp,mngrAndDidirect,ans);
		}
		ans.put(ceo,cnt);
		return cnt+1;
	}
